  ...
  class SitemapGenerator < Generator
      ...
      File.open(File.join(site.dest, 'sitemap.xml'), 'w+') do |sitemap|
        ...
        locs['en']['url'].each do |id,value|
          locs.each do |lang,value|
            ...
            sitemap.puts '<url>'
            sitemap.puts '  <loc>https://bitcoin.com/'+lang+'/'+CGI::escape(locs[lang]['url'][id])+'</loc>'
            locs.each do |altlang,value|
              ...
              sitemap.puts '    href="https://bitcoin.com/'+altlang+'/'+CGI::escape(locs[altlang]['url'][id])+'" />'
            end
            sitemap.puts '</url>'
          end
        end
	#Add static non-translated pages
        Dir.foreach('.') do |file1|
          if /^[a-z]{2}(_[A-Z]{2})?$/.match(file1) and File.directory?(file1)
            Dir.foreach(file1) do |file2|
              ...
              sitemap.puts '<url>'
              sitemap.puts '  <loc>https://bitcoin.com/'+file1+'/'+file2.gsub('.html','').gsub('.md','')+'</loc>'
              sitemap.puts '</url>'
            end
          end
          ...
          sitemap.puts '<url>'
          sitemap.puts '  <loc>https://bitcoin.com/'+file1.gsub('.html','').gsub('.md','')+'</loc>'
          sitemap.puts '</url>'
        end
        ...
        Dir.foreach('_alerts') do |file|
          ...
          sitemap.puts '<url>'
          sitemap.puts '  <loc>https://bitcoin.com/en/alert/'+file.gsub('.html','')+'</loc>'
          sitemap.puts '</url>'
        end
        ...
        Dir.foreach('_releases') do |file|
          ...
          file = file.join('-')
          sitemap.puts '<url>'
          sitemap.puts '  <loc>https://bitcoin.com/en/release/'+file.gsub('.md','').gsub('.html','')+'</loc>'
          sitemap.puts '</url>'
        end
        #Close sitemap
        sitemap.puts '</urlset>'
